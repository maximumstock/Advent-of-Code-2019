...
fn part1() {
    ...
    let mut cable1_coordinates = vectors_to_points(cable1_vectors);
    let mut cable2_coordinates = vectors_to_points(cable2_vectors);

    cable1_coordinates.sort();
    cable2_coordinates.sort();

    let intersections: Vec<Point> = find_intersections(&cable1_coordinates, &cable2_coordinates);
...
fn find_intersections(cable1: &Vec<Point>, cable2: &Vec<Point>) -> Vec<Point> {
...
fn vectors_to_points(vectors: Vec<&str>) -> Vec<Point> {
    ...
    for vector in vectors {
        let starting_point: Point = *points.last().unwrap();
        let (x_range, y_range) = parse_vector(vector);
        for x in x_range {
            let new_point = (starting_point.0 + x, starting_point.1);
            points.push(new_point)
        }
        for y in y_range.clone() {
            let new_point = (starting_point.0, starting_point.1 + y);
            points.push(new_point)
        }
    }
...
fn parse_vector(vector: &str) -> (Range<i32>, Range<i32>) {
...
fn input() -> (Vec<&'static str>, Vec<&'static str>) {
    ...
    let cable1 = cable1.split(", ").collect::<Vec<&str>>();
    let cable2 = cable2.split(", ").collect::<Vec<&str>>();
    (cable1, cable2)
}

#[test]
fn test_vectors_to_points() {
    assert_eq!(vectors_to_points(vec!["D1", "R1", "L1", "U1"]), vec![(0, 0), (0, -1), (1, -1), (0, -1), (0, 0)])
}
